...
| D9 | `loombox-brand` memory, 2026-07-21 (issues #194/#195/#196/#377) | Which pitched logo concept becomes the mark, and which accent replaces the two rejected first-round colors? | Concept A "Warp & Weft" (a 2×2 plain-weave glyph, real over-under via stroke gaps, in a rounded-square frame) over B "Closed Loop" and C "Bracket Box"; azure `#3b9df7`/`#1f7fd0` over a rejected coral (too close to Claude Code's own orange) and brass (flat), landing as a 6-preset + custom-hex theme system rather than one fixed accent. Current values live in code (`accent-presets.ts`, `BrandMark.svelte`), not here — this row is only the origin record D2 assumes. | |
