...
describe('relay v1', () => {
  ...
  describe('git_hunk_diff_request/git_hunk_diff_response (issue #232) — routed and fanned out exactly like git_diff_request/git_diff_response, always blind, no envelope on the request', () => {
    it('routes a client git_hunk_diff_request to the node owning that session, byte-for-byte, never inspecting the envelope', async () => {
      const { url, close } = await startRelay({ host: '127.0.0.1', port: 0 });
      closers.push(close);
      ...
      const meta = makeSessionMeta({ id: 'sess_git_hunk_diff', accountId: 'acct_1' });
      send(node, {
        type: 'session_announce',
        protocolVersion: PROTOCOL_V1,
        session: meta,
      ...
      const request: GitHunkDiffRequest = {
        type: 'git_hunk_diff_request',
        protocolVersion: PROTOCOL_V1,
        sessionId: 'sess_git_hunk_diff',
        requestId: 'req_1',
      };
      send(client, request);

      const received = (await nextMessage(node)) as unknown as GitHunkDiffRequest;
      expect(received).toEqual(request);
      expect(Object.keys(received).sort()).toEqual(
        ['protocolVersion', 'requestId', 'sessionId', 'type'].sort(),
      );
    ...
    it('ignores a git_hunk_diff_request for an unknown session instead of throwing', async () => {
      const { url, close } = await startRelay({ host: '127.0.0.1', port: 0 });
      closers.push(close);
    ...
    it("fans git_hunk_diff_response out to the session's subscribed client, byte-for-byte, never inspecting the envelope", async () => {
      const { url, close } = await startRelay({ host: '127.0.0.1', port: 0 });
      closers.push(close);
      ...
      const meta = makeSessionMeta({ id: 'sess_git_hunk_diff_reply', accountId: 'acct_1' });
      send(node, {
        type: 'session_announce',
        protocolVersion: PROTOCOL_V1,
        session: meta,
      ...
      send(client, {
        type: 'session_resume',
        sessionId: 'sess_git_hunk_diff_reply',
        protocolVersion: PROTOCOL_V1,
      } satisfies SessionResume);
      await nextMessage(client); // the session_announce reply from resume

      const response: GitHunkDiffResponse = {
      ...
      const received = (await nextMessage(client)) as unknown as GitHunkDiffResponse;
      expect(received).toEqual(response);
      expect(Object.keys(received).sort()).toEqual(
        ['envelope', 'protocolVersion', 'requestId', 'sessionId', 'type'].sort(),
      );
  ...
  describe('git_hunk_action_request/git_hunk_action_response (issue #232) — routed and fanned out exactly like fs_read_request/fs_read_response, always blind, envelope on both request and response', () => {
    it('routes a client git_hunk_action_request to the node owning that session, byte-for-byte, never inspecting the envelope', async () => {
      const { url, close } = await startRelay({ host: '127.0.0.1', port: 0 });
      closers.push(close);
      ...
      const meta = makeSessionMeta({ id: 'sess_git_hunk_action', accountId: 'acct_1' });
      send(node, {
        type: 'session_announce',
        protocolVersion: PROTOCOL_V1,
        session: meta,
      ...
      const request: GitHunkActionRequest = {
        ...
        protocolVersion: PROTOCOL_V1,
        sessionId: 'sess_git_hunk_action',
        requestId: 'req_1',
        envelope: fakeEnvelope('{"path":"src/index.ts","hunkIndex":0,"action":"stage"}'),
      };
      send(client, request);

      const received = (await nextMessage(node)) as unknown as GitHunkActionRequest;
      expect(received).toEqual(request);
      expect(Object.keys(received).sort()).toEqual(
        ['envelope', 'protocolVersion', 'requestId', 'sessionId', 'type'].sort(),
      );
    ...
    it('ignores a git_hunk_action_request for an unknown session instead of throwing', async () => {
      const { url, close } = await startRelay({ host: '127.0.0.1', port: 0 });
      closers.push(close);
      ...
      send(client, {
        ...
        requestId: 'req_orphan',
        envelope: fakeEnvelope('{"path":"src/index.ts","hunkIndex":0,"action":"stage"}'),
      } satisfies GitHunkActionRequest);
    ...
    it("fans git_hunk_action_response out to the session's subscribed client, byte-for-byte, never inspecting the envelope", async () => {
      const { url, close } = await startRelay({ host: '127.0.0.1', port: 0 });
      closers.push(close);
      ...
      const meta = makeSessionMeta({ id: 'sess_git_hunk_action_reply', accountId: 'acct_1' });
      send(node, {
        type: 'session_announce',
        protocolVersion: PROTOCOL_V1,
        session: meta,
      ...
      send(client, {
        type: 'session_resume',
        sessionId: 'sess_git_hunk_action_reply',
        protocolVersion: PROTOCOL_V1,
      } satisfies SessionResume);
      await nextMessage(client); // the session_announce reply from resume

      const response: GitHunkActionResponse = {
      ...
      const received = (await nextMessage(client)) as unknown as GitHunkActionResponse;
      expect(received).toEqual(response);
      expect(Object.keys(received).sort()).toEqual(
        ['envelope', 'protocolVersion', 'requestId', 'sessionId', 'type'].sort(),
      );
