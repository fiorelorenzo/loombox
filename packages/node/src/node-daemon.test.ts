    ...
    expect(initialCatalog!.options).toEqual([
      {
        ...
        category: 'mode',
        current: 'default',
        // Each choice's `description` is the agent's own, and issue #897 is that it used
        // to be dropped between the wire and the client. Asserting it at the node's own
        // wire boundary is what proves it survives the whole way, not just the mapping.
        choices: [
