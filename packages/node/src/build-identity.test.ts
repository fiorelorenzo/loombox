...
describe('readNodeBuildIdentity', () => {
  ...
  it('prefers LOOMBOX_BUILD_COMMIT over git — the future staged-artifact path (#658/#86), where a checkout is not guaranteed', async () => {
  ...
  it("falls back to git rev-parse when LOOMBOX_BUILD_COMMIT is unset — today's real case: tsx src/main.ts run from a git checkout", async () => {
    const identity = await readNodeBuildIdentity({
      env: {},
