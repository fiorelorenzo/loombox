...
export class NodeDaemon extends EventEmitter {
  ...
  private async launchLocalSession(
    ...
    try {
      ...
      // D1-3 (issue #748): a custom-agent allowlist refusal forwards its
      // own message as this session_status's `reason`, so the client shows
      // the real refusal rather than a bare "error" — every other spawn
      // failure keeps the pre-#748 behaviour (no reason).
      const reason = error instanceof CustomAgentNotAllowedError ? error.message : undefined;
      await this.sendSessionStatus(session.id, 'error', reason).catch((sendError: unknown) => {
        console.warn(
   ...
   * reported when that spawn times out (issue #516) or is refused by the
   * custom-agent allowlist (issue #748); and `'disconnected'` (issue
   * #702), re-sent on every reconnect (see {@link reannounceAll}) for
   * every session `SessionManager` reports in that state — a disconnected
   * session never gets a bridge again on its own, so nothing else would
   * ever push this one. Every other status transition rides
   ...
   *
   * `reason` rides along only for the refusals a client must see verbatim
   * (today: {@link CustomAgentNotAllowedError}'s own message) — omit it for
   * every ordinary transition, exactly as every caller before issue #748
   * already did.
   */
